...
secured = APIRouter(
    dependencies=[
        Depends(require_api_key),           # Require API key authentication
        Depends(verify_signature_if_present),  # Optional HMAC signature verification
        Depends(RateLimiter(times=60, seconds=60)),  # 60 requests per minute per API key
    ]
)
